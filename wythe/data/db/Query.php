...
class Query{
	...
	/*将sql语句中的__TABLE_NAME__字符串替换成带前缀的表名*/
	...
	/*执行查询 返回数据集*/
	public function query($sql,$bind=[],$master = false,$calss=false){
	...
	public function max($field,$force = true){
		return $this->value('MAX(' . $field .') AS to_max',0,$force);
	}
	...
	public function setDec($field,$step = 1, $lazyTime = 0){
		...
		if($lazyTime > 0){
			$guid = md5($this->getTable() . '_' . $field . '_' . serialize($condition) . serilize($this->bind));
	...
	/*查询sql组装 union*/
	public function join($union,$all = false){
	...
	/*指定查询字段 支持字段排除和指定数据表*/
	public function field($field,$except = fale , $tableName = '', $prefix = '',$alias = ''){
		if(empty($field)){
			return $this;
	...
	*/
	...
	public function limit($offset,$length = null){
		if(is_null($length) && strpos($offset,',')){
			list($offst,$length) = explode(',',$offset);
		}
	...
	public function cache($key = true, $expire =null ,$tag = null){
		...
		if(false !== $key){
			$this->options['cache'] = ['key'=>$key,'expore'=>$expire,'tag'=>$tag];
		}
	...
	/*指定distinct查询*/
	public function distict($distinct){
	...
	public function getTableInfo($tableName = '',$fetch = ''){
		...
		return $fetch ? self::$info[$db . '.' . $guid[$fetch] : self::$inf[$db . '.' . $guid]];
	}
	...
	public function getPk($options = ''){
		if(!empty($this->pk)){
			...
			$pk = $this->getTableInfo(is_array($options) ? $options['table'] : $optionsl,'pk');
		}
		return $pk;
	}
	...
	public function insertAll(array $dataSet,$replace = false ,$limit = null){
		...
		if(is_null($limit)){
			...
			foreach ($array as $item) {
				$sql[] = $this->builder->insertAll($item,$options,$relace);
			}
		}
	...
	public function select($data = null){
		...
		if(!empty($this->model)){
			if(count($resultSet) > 0){
				foreach ($reslutSet as $key => $result) {
	...
	public function delete($data = null){
		...
		if(isset($options['cahce']) && is_string($options['cache']['key'])){
			$key = $Options['cache']['key'];
		}
		...
		if(isset($key) && Cache::get($key)){
			Cache::rm($key);
		} elseif (!empty($options['cachce']['tag'])){
			Cache::clear($options['cache']['tag']);
		}
